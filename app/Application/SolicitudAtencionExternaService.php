    ...
    protected function setQueryFilters($query, $filter)
    {
        if(($numero = Arr::get($filter, "numero")) && is_numeric($numero)) {
            ...
            if (($proveedorId = Arr::get($filter, "proveedor"))) {
                $query->whereRaw("MATCH(`proveedor`) AGAINST(? IN BOOLEAN MODE)", [$proveedorId . "*"]);
                // $query->where("proveedor", "LIKE", $proveedorId."%");
            }
            if (($medicoId = Arr::get($filter, "medico"))) {
                $query->whereRaw("MATCH(`medico`) AGAINST(? IN BOOLEAN MODE)", [$medicoId . "*"]);
                // $query->where("medic", "LIKE", $medicoId."%");
            }
