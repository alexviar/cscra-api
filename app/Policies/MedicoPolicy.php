...
class MedicoPolicy
{
    ...
    public function cambiarEstado(User $user, $medico, $estado){
    //   if($estado == 1) {
    //       if($user->can(Permisos::BAJA_MEDICOS)) return true;
    //       if($user->can(Permisos::BAJA_MEDICOS_REGIONAL) && $user->regional_id == $medico->regional_id) return true;
    //   }
    //   else if($estado == 2){
        if($user->can(Permisos::BAJA_MEDICOS)) return true;
        if($user->can(Permisos::BAJA_MEDICOS_REGIONAL) && $user->regional_id == $medico->regional_id) return true;
